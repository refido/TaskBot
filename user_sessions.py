...
class XHRTracker:
    """Capture XHR/fetch traffic plus request/response bodies via CDP."""
...
def _datetime_to_rfc1123(value: datetime | None) -> str | None:
    if value is None:
        return None
...
def _chromium_timestamp_to_datetime(value: Any) -> datetime | None:
    if value in (None, "", 0):
        return None
...
def _normalize_cookie_key(name: str, domain: str, path: str) -> tuple[str, str, str]:
    normalized_domain = domain.lstrip(".").lower()
    return (name, normalized_domain, path)


def _cookie_key_from_mapping(mapping: dict[str, Any], domain_key: str = "domain") -> tuple[str, str, str]:
    return _normalize_cookie_key(
...
def _capture_browser_state(page: Page) -> dict[str, Any]:
...
def _read_sqlite_cookie_rows(cookie_db_path: Path) -> list[dict[str, Any]]:
...
def _build_enriched_sqlite_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    enriched_rows: list[dict[str, Any]] = []

    for row in rows:
        created_at = _chromium_timestamp_to_datetime(row.get("creation_utc"))
        last_accessed = _chromium_timestamp_to_datetime(row.get("last_access_utc"))
        updated_at = _chromium_timestamp_to_datetime(row.get("last_update_utc"))
        expires_at = _chromium_timestamp_to_datetime(row.get("expires_utc"))

        enriched_row = dict(row)
        enriched_row["created"] = _datetime_to_rfc1123(created_at)
        enriched_row["created_iso"] = created_at.isoformat() if created_at else None
        enriched_row["last_accessed"] = _datetime_to_rfc1123(last_accessed)
        enriched_row["last_accessed_iso"] = (
            last_accessed.isoformat() if last_accessed else None
        )
        enriched_row["updated"] = _datetime_to_rfc1123(updated_at)
        enriched_row["updated_iso"] = updated_at.isoformat() if updated_at else None
        enriched_row["expires_max_age"] = _datetime_to_rfc1123(expires_at)
...
def _build_enriched_cookies(
    playwright_cookies: list[dict[str, Any]],
    cdp_cookies: list[dict[str, Any]],
    sqlite_rows: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    playwright_by_key = {
        _cookie_key_from_mapping(cookie): cookie for cookie in playwright_cookies
    }
    cdp_by_key = {_cookie_key_from_mapping(cookie): cookie for cookie in cdp_cookies}
    sqlite_by_key = {
        _cookie_key_from_mapping(cookie, domain_key="host_key"): cookie
    ...
    for collection, domain_key in (
        ...
        for cookie in collection:
            key = _cookie_key_from_mapping(cookie, domain_key=domain_key)
            if key in seen_keys:
                continue
            seen_keys.add(key)
    ...
    for key in ordered_keys:
        ...
        created_at = _chromium_timestamp_to_datetime(sqlite_cookie.get("creation_utc"))
        last_accessed = _chromium_timestamp_to_datetime(
            sqlite_cookie.get("last_access_utc")
        )
        updated_at = _chromium_timestamp_to_datetime(
            sqlite_cookie.get("last_update_utc")
        )
        expires_at = _chromium_timestamp_to_datetime(sqlite_cookie.get("expires_utc"))
        if expires_at is None:
            expires_at = _unix_seconds_to_datetime(cdp_cookie.get("expires"))
        if expires_at is None:
            expires_at = _unix_seconds_to_datetime(playwright_cookie.get("expires"))
        ...
        enriched.append(
            {
                ...
                "domain": domain,
                "path": path,
                "created": _datetime_to_rfc1123(created_at),
                "created_iso": created_at.isoformat() if created_at else None,
                "expires_max_age": _datetime_to_rfc1123(expires_at),
                ...
                "last_accessed": _datetime_to_rfc1123(last_accessed),
                "last_accessed_iso": (
                    last_accessed.isoformat() if last_accessed else None
                ),
                "same_site": (
                ...
                "size": size,
                "updated": _datetime_to_rfc1123(updated_at),
...
def _log_cookie(operator: str, index: int, cookie: dict[str, Any]) -> None:
...
def _log_xhr_summary(operator: str, xhr_entries: list[dict[str, Any]]) -> None:
...
def _build_operator_paths(output_dir: Path, operator: str) -> dict[str, Path]:
    operator_slug = _safe_slug(operator)
...
def export_account_session(config: Config, output_dir: Path) -> SessionArtifacts:
    ...
    paths = _build_operator_paths(output_dir, operator)
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            web_storage = _capture_browser_state(page)
    ...
    sqlite_rows = _build_enriched_sqlite_rows(_read_sqlite_cookie_rows(cookie_db_path))
    enriched_cookies = _build_enriched_cookies(
        playwright_cookies=playwright_cookies,
        cdp_cookies=cdp_cookies,
        sqlite_rows=sqlite_rows,
    )

    _write_json(paths["cookies_playwright"], playwright_cookies)
    _write_json(paths["cookies_cdp"], cdp_cookies_payload)
    _write_json(paths["cookies_sqlite"], sqlite_rows)
    _write_json(paths["cookies_enriched"], enriched_cookies)
    _write_json(paths["storage_state"], storage_state)
    _write_json(paths["web_storage"], web_storage)
    _write_json(paths["xhr"], xhr_entries)
    _write_json(paths["session_summary"], session_summary)
    ...
    for index, cookie in enumerate(enriched_cookies, start=1):
        _log_cookie(operator, index, cookie)

    _log_xhr_summary(operator, xhr_entries)
...
def main() -> int:
    logging_meta = configure_logging(app_name="user_sessions")
    logger.bind(
    ...
    selected_account_configs = account_configs[:MAX_ACCOUNT_HITS_PER_RUN]
    if len(account_configs) > MAX_ACCOUNT_HITS_PER_RUN:
        log_print(
            (
                f"Configured {len(account_configs)} accounts, "
                f"but this run is limited to the first {MAX_ACCOUNT_HITS_PER_RUN} "
            ...
            max_account_hits=MAX_ACCOUNT_HITS_PER_RUN,
        )

    output_dir = _build_output_dir()
    log_print(
        f"Session artifacts directory: {output_dir.resolve()}",
    ...
    for index, account_config in enumerate(selected_account_configs):
        ...
        try:
            artifacts.append(export_account_session(account_config, output_dir))
        except Exception:
        ...
        if not is_last_account:
            log_print(
                (
                    f"Cooling down for {INTER_ACCOUNT_COOLDOWN_SECONDS} seconds "
                    f"before the next account to avoid bursting the app."
                ),
                event="user_sessions.cooldown",
                cooldown_seconds=INTER_ACCOUNT_COOLDOWN_SECONDS,
            )
            time.sleep(INTER_ACCOUNT_COOLDOWN_SECONDS)
